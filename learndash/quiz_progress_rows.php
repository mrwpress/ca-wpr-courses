	...
	if ( ! empty( $quiz_title ) ) : ?>
      <p id="ld-quiz-<?php echo $v['time']; ?>">
        <strong><a href="<?php echo learndash_get_step_permalink( $quiz->ID, $quiz_course_id ) ?>"><?php echo $quiz_title ?></a></strong>
		  ...
		  <?php
		  if ( ( ( $user_id == get_current_user_id() ) || ( learndash_is_admin_user() ) || ( learndash_is_group_leader_user() ) ) && ( ! empty( $certificateLink ) ) ) {
			  $final_quiz_id         = get_option( 'wpr_courses_settings_quiz_id' );
			  $witobaccocheck_status = get_user_meta( $user_id, '_wpr_witobaccocheck_status', 'false' );

			  if ( learndash_is_admin_user() || ( $final_quiz_id == $v['quiz'] && 'true' === $witobaccocheck_status ) ) {
				  if (
        ...
        <br/>
		  ...
		  if ( isset( $v['rank'] ) && is_numeric( $v['rank'] ) ) :
			  echo __( 'Rank: ', 'learndash' ) . ' ' . $v['rank'];
		  endif;

		  echo __( 'Score ', 'learndash' ); ?><?php echo $v['score']; ?> <?php echo __( ' out of ', 'learndash' ); ?> <?php
		   ...
		   */
		  ?>
		  <?php do_action( 'course_info_shortcode_after_item', $quiz, $v, $user_id ); ?>
      </p>
	<?php endif; ?>
<?php endforeach; ?>
